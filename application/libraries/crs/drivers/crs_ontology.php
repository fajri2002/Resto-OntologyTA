...
class CRS_Ontology extends CI_Driver 
{
	...
	function has_ratings($var)
	{
		$sparql = 'SELECT ?rating WHERE { data:'.$var.' data:hasRatings ?rating }';
	...
	function get_price($pref)
	{
		// Start constructing the SPARQL query
		$sparql = 'SELECT DISTINCT ?prod WHERE { 
			?prod rdf:type data:Product . 
			?prod data:hasPrice ?price .';

		// Check if 'price' is set and not empty in the preferences array
		if (isset($pref['price']) && !empty($pref['price']))
		{
		// Iterate through each price range in the 'price' preference
		foreach ($pref['price'] as $price)
			// Add each price range to the SPARQL query with a UNION clause
			$sparql .= ' FILTER(?price = "' . $price . '") UNION ';

		// Remove the trailing 'UNION ' from the query
		$sparql = substr($sparql, 0, -6);
		}

		// Close the WHERE clause and add an ORDER BY clause
		$sparql .= '} ORDER BY ?prod';

		// Execute the constructed SPARQL query and return the result
		return $this->execute($sparql, 1);
	}
