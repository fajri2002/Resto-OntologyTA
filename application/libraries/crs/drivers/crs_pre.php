...
class CRS_PRE extends CI_Driver
{
	...
	function Recommend($usermodel, $pref)
	{
		...
		$price = $this->ontology->get_price($pref);

		$product_filtered1 = array_intersect($type, $price);
	...
	function explain($product, $usermodel, $pref)
	{
		...
		foreach($product as $id => $prod)
		{
			...
			$ratings =  $this->ontology->has_ratings($prod['produk']);
			$detail	= $this->ontology->has_detail($prod['produk']);
			$type	= $this->_get_type($prod['produk'], $prod_type);
			$txt = 'Resto '.str_replace('_', ' ', $prod['produk']).' adalah tempat makan yang berlokasi di '.$type.' dengan '.$price.' . Yang mendukung kebutuhan seperti : <ul>';
	...
	function recommendSR($pref, $pref2)
	{
		...
		if(empty($pref))
		{
			...
			foreach($result2 as $prod)
			{
				...
				$return[$id]['rating']	= $this->ontology->has_ratings($prod);
				...
				$return[$id]['explain'] = 'Harga '.$this-> $price;
	...
	function explainSR($result)
	{
		...
		if(!empty($result))
		{
			...
			foreach($result as $prod)
			{
				...
				$return[$id]['explain'] = 'Harga '.$this->$price; /*ini*/
